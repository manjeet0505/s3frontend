...
function SessionRequestModal({ mentor, studentId, studentName, onClose, onSuccess }) {
  ...
  const [useCalendly, setUseCalendly] = useState(!!mentor?.calendly_url);
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/mentor-requests', {
        ...
        body: JSON.stringify({
          // FIX: use mongo_id as mentor_id so it matches mentor's JWT userId
          mentor_id: mentor.mongo_id || mentor.user_id || mentor.email,
          mentor_user_id: mentor.mongo_id || mentor.user_id,
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-md rounded-3xl border border-border/60 bg-card/95 backdrop-blur-xl shadow-2xl p-6 overflow-hidden max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        {/* Calendly toggle if available */}
        {mentor.calendly_url && (
          <div className="relative mb-5">
            <div className="flex gap-2 p-1 rounded-xl bg-secondary/40 border border-border/40">
              <button
        ...
        {/* Calendly embed */}
        {useCalendly && mentor.calendly_url ? (
          <div className="relative space-y-4">
            <div className="p-4 rounded-xl bg-blue-500/8 border border-blue-500/20">
              <p className="text-xs text-blue-400 font-medium mb-1">Book via Calendly</p>
              <p className="text-xs text-muted-foreground mb-3">
                Click below to open {mentor.name}'s Calendly and book a time that works for both of you.
              </p>
              <a
                href={mentor.calendly_url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 transition-all shadow-lg shadow-primary/25"
              >
            ...
            <p className="text-xs text-muted-foreground text-center">
              Or switch to Custom Slots to book using their set availability
            </p>
          </div>
        ) : (
          /* Custom slot booking */
    ...
    >
      ...
      <div className="relative">
        ...
        >
          ...
          {mentor.calendly_url ? 'Book Session' : 'Request Session'}
        </motion.button>
      </div>
    </motion.div>
  );
